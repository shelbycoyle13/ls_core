...
# Sometimes it is necessary to define a custom data structure of some type. In some languages, including Python, there is a built-in Set or set type. For this problem, you're expected to implement your own custom set type: you may not use the built-in set type.

# How this type works internally doesn't matter, as long as it behaves like a set of unique elements that can be manipulated in several well defined ways. Once you've reached a solution, feel free to play around with using the built-in implementation of set.
